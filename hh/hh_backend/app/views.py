...
class SavedJobView(APIView):
    ...
    def post(self, request):
        ...
        # Optionally return the saved job data
        serializer = JobLimitedSerializer(job)
        return Response({
...
class NewSaveJobView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        saved_jobs = NewSaveJob.objects.filter(user=request.user)
        serializer = NewSaveJobSerializer(saved_jobs, many=True)
        data = {
            "count": saved_jobs.count(),
            "jobs": serializer.data
        }
    ...
    def post(self, request):
        print("Received POST request to save job")
        ...
        # Use the job ID from the API as our primary key
        job_id = request.data.get('job_id')
        if not job_id:
        ...
        existing_job = NewSaveJob.objects.filter(id=job_id, user=request.user).first()
        if existing_job:
            return Response({
                'message': 'Job is already saved',
                'job': NewSaveJobSerializer(existing_job).data
            }, status=status.HTTP_200_OK)
        
        serializer = NewSaveJobSerializer(data=data)
        if serializer.is_valid():
...
class NewSaveJobDetailView(APIView):
    permission_classes = [IsAuthenticated]

    def delete(self, request, job_id):
        try:
            saved_job = NewSaveJob.objects.get(user=request.user, id=job_id)
            saved_job.delete()
            return Response({'message': 'Job removed successfully'}, status=status.HTTP_204_NO_CONTENT)
        except NewSaveJob.DoesNotExist:
            return Response({'error': 'Saved job not found'}, status=status.HTTP_404_NOT_FOUND)

class MessageChannelListView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        channels = MessageChannel.objects.filter(user=request.user).order_by('-last_message_time')
    ...
    def post(self, request):
        ...
        serializer = MessageChannelSerializer(channel)
        status_code = status.HTTP_201_CREATED if created else status.HTTP_200_OK
        return Response(serializer.data, status=status_code)

class MessageChannelDetailView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, job_id):
        try:
            channel = MessageChannel.objects.get(job_id=job_id, user=request.user)
            serializer = MessageChannelSerializer(channel)
            return Response(serializer.data)
...
class MessageListView(APIView):
    ...
    def get(self, request, channel_id):
        try:
            channel = MessageChannel.objects.get(job_id=channel_id, user=request.user)
            ...
            return Response({
                'channel': MessageChannelSerializer(channel).data,
    ...
    def post(self, request, channel_id):
        try:
            channel = MessageChannel.objects.get(id=channel_id, user=request.user)
            ...
            message = Message.objects.create(
                ...
                sender=request.user,
                content=content,
                is_read=True  # Message is read by sender by default
            )

            # Update channel's last message time
            channel.last_message_time = message.created_at
            channel.save()
