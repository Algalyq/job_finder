...
class MessageChannel(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='message_channels')
    ...
    def __str__(self):
        return f"Channel between {self.user.username} and {self.company} for {self.jobtitle}"
...
class Message(models.Model):
    ...
    def __str__(self):
        return f"Message from {self.sender.username} in {self.channel}"
...
class NewSaveJob(models.Model):
    id = models.CharField(max_length=255, primary_key=True)
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='new_saved_jobs')
    title = models.CharField(max_length=100)
    company = models.CharField(max_length=100)
    ...
    def __str__(self):
        return f"{self.user.username} saved {self.title} at {self.company}"
